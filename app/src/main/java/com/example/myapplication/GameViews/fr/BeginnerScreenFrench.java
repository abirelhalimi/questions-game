...
public class BeginnerScreenFrench extends Screen {
    ...
    public BeginnerScreenFrench(Game game) throws IOException {
        super(game);
        ...
        Sounds.playSound(Sounds.Questions_who_fr[Images.Nums[Images.index]]);

        background = new StartingSprite(game, Images.background_img, 0, 0, game.getGraphics().getHeight(), game.getGraphics().getWidth());
        qst = new Resp(game, Images.Questions_fr[Images.Nums[Images.index]], Sounds.Questions_who_fr[Images.Nums[Images.index]], 0, 0, (int) (game.getGraphics().getHeight() / (2.24)), (int) (game.getGraphics().getWidth()));
        rsp = new Resp(game, Images.Answers[Images.Nums[Images.index]], Sounds.correct_audio_fr, Images.position[0], (int) (game.getGraphics().getHeight() / (1.68)), (int) (game.getGraphics().getHeight() / (5.334)), (int) (game.getGraphics().getWidth() / (3)));
        rsp1 = new Resp(game, Images.Others[0], Sounds.tryagain_audio_fr, Images.position[1], (int) (game.getGraphics().getHeight() / (1.68)), (int) (game.getGraphics().getHeight() / (5.334)), (int) (game.getGraphics().getWidth() / (3)));
        rsp2 = new Resp(game, Images.Others[2], Sounds.tryagain_audio_fr, Images.position[2], (int) (game.getGraphics().getHeight() / (1.68)), (int) (game.getGraphics().getHeight() / (5.334)), (int) (game.getGraphics().getWidth() / (3)));
    ...
    public void handleTouchDown(int x, int y, int pointer) {
        ...
        if (s != null && s.equals(home)) {
            ...
            Sounds.stop();
            game.setScreen(new StartingScreenFrench((MainActivityEng) game));
        }
        if (s != null && s.equals(next)) {

            try {
                game.setScreen(new BeginnerScreenFrench((MainActivityEng) game));
            } catch (IOException e) {
                e.printStackTrace();
            }
        }
